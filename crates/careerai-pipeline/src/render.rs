//! Render stage: convert a tailored application into DOCX + PDF artifacts.
...
use crate::{load_profile, open_pool, RenderedOutcome};
...
pub async fn render_one(
    ...
    // Attach artifact rows for each rendered file.
    for (kind, path) in [
