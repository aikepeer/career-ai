//! Match stage: filter + score discovered listings against the profile.
...
use crate::{load_profile, open_pool, MatchReport};

/// Fire a `HighScoreMatch` notification when `score >= threshold`.
pub(crate) async fn fire_high_score_if_above(
    pipe: &NotifyPipeline,
    listing_id: &str,
    title: &str,
    company: &str,
...
/// Delegates to `match_all` since matching is naturally global.
