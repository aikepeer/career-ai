...
static CATALOG: &[CliCommandCard] = &[
    ...
    CliCommandCard {
        group: "Inspect",
        name: "patterns",
        argv: "careerai patterns",
        summary: "Analyze reposts/ghost-jobs, funnel velocity, and rejection latency.",
        hint: "Detects reposted listings, per-source advance rates, and ghosted-then-rejected timelines.",
        availability: CliAvailability::Runnable,
        run_command: Some("patterns"),
    },
    CliCommandCard {
        group: "Inspect",
        name: "upskill",
        argv: "careerai upskill",
        summary: "Skill gap analysis between your profile and shortlisted JDs.",
        hint: "Shows strengths (skills you have that JDs want) and gaps (skills JDs want that you lack).",
        availability: CliAvailability::Runnable,
        run_command: Some("upskill"),
    },
    CliCommandCard {
        group: "Inspect",
        name: "analyze-profile",
        argv: "careerai analyze-profile",
        summary: "Audit your profile for ATS readiness and quantification.",
        hint: "Checks contact info, date formats, bullet quantification rate, and skill density.",
        availability: CliAvailability::Runnable,
        run_command: Some("analyze-profile"),
    },
    CliCommandCard {
        group: "Sources",
        name: "sources sync",
    ...
    CliCommandCard {
        group: "Diagnose",
        name: "interview",
        argv: "careerai interview <listing-id>",
        summary: "Generate an interview prep pack (STAR stories + likely questions).",
        hint: "Extracts STAR stories from your profile and maps likely JD questions to them.",
        availability: CliAvailability::NeedsId,
        run_command: Some("interview"),
    },
    CliCommandCard {
        group: "Diagnose",
        name: "email",
        argv: "careerai email <listing-id>",
        summary: "Draft a cold application email for a listing (draft-only).",
        hint: "Generates a subject + body + attachment checklist. Never sends — review and send manually.",
        availability: CliAvailability::NeedsId,
        run_command: Some("email"),
    },
    CliCommandCard {
        group: "Diagnose",
        name: "negotiate",
        argv: "careerai negotiate <listing-id> [--benchmark ...]",
        summary: "Generate a salary negotiation script (counter-offer + talking points).",
        hint: "Anchored on your profile's compensation target. Counter 10-20% above the initial offer.",
        availability: CliAvailability::NeedsId,
        run_command: Some("negotiate"),
    },
    CliCommandCard {
        group: "Credentials",
